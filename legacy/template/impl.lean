/-
# ThingFn — the guest implementation surface
...
/-- The `thing-get` implementation: none for the sentinel id, a real
    record otherwise (the GuestImpl pattern's minimal mirror). -/
@[guest_std, schema_fn]
def thing_get (id : UInt64) : Option Thing :=
  if id == 0 then none
  else some { id := id, label := "thing" }
