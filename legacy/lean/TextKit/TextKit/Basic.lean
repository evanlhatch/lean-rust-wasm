/-
# TextKit.Basic — the parser monad, escapes, name scanners, prefix kit

Foundation lift OUT of substrait: the substrait-AGNOSTIC parser machinery
that lived in `Substrait.Decode.Basic` (its W5.3 phase-1 split of the
monolithic Decode.lean) re-homed core-only, so future consumers (the
snapshot-text surface, the gates' manifest parser, a future query
language's parser) can reach it without depending on substrait.
Pure code-motion — bodies and theorem statements are unchanged modulo the
namespace (`Substrait.Decode.*` → `TextKit.*`); the parsers' BEHAVIOR is
pinned by substrait's goldens/sweeps (byte-tie law: zero behavior change).

Provenance of the four text primitives (`isIdentChar`, `isIdentifier`,
`escape`, `name`): lifted from `Substrait.Emit.Text` — they were that
module's only non-core content and the inversion kit's only dependency.
substrait re-exports BOTH moved surfaces under their historical names
(`Substrait.Emit.Text.escape`/`name`/…, `Substrait.Decode.Parser`/… — same
constants, not copies), so no downstream file churns.

STAYS in substrait (grammar-specific, deliberately not moved): the
token/name tables (`Substrait.Grammar`'s urn/join/set/sort rows, every
`*Tok` constant) and everything above the four primitives in the emitter
(`sep`, `fieldRef`, the type/rel renderers). The escape table
(`\n \t \r \\ " '`) is defined HERE as the generic text-escape contract;
`escapeArm_eq`/`escape_toList` pin it arm-for-arm.

CORE-ONLY: zero imports — kernel-package discipline (this package sits
below substrait; the proofs reduce through core + these defs alone).
-/
...
/-! ## the text primitives (lifted from substrait's Emit.Text) -/
...
/-- Escape a string for quoted identifiers and string literals
    (textify `escaped` — the table above IS the contract). -/
...
/-! ## the escape inversion kit -/

/-- The character text `escape` writes for one character, as a
...
/-- The escape, as a list fold over `escChars`. -/
theorem escape_toList (s : String) :
    (escape s).toList = s.toList.flatMap escChars := by
...
theorem unescape_escape (s : String) : unescape (escape s) = some s := by
...
where
  scanIdentGo : Parser String := do
    let h ← Parser.peek
    if h.isAlpha then Parser.takeWhile isIdentChar
    else Parser.fail
...
theorem scanIdent_of_identifier (n : String) (h : isIdentifier n = true)
  ...
  | cons c tail =>
    ...
    rw [show scanIdent.scanIdentGo (c :: (tail ++ rest)) =
        Parser.takeWhile isIdentChar (c :: (tail ++ rest)) by
...
theorem scanName_quoted (n : String) (rest : List Char) :
    scanName ('"' :: (escape n).toList ++ '"' :: rest) = some (n, rest) := by
  unfold scanName scanName.scanNameGo
  let X : List Char := (escape n).toList ++ '"' :: rest
...
/-- **The name inversion**: `name` scans back to the name. -/
theorem scanName_name (n : String) (rest : List Char)
    (hsep : rest = [] ∨ ∃ c0 rest0, rest = c0 :: rest0 ∧ isIdentChar c0 = false) :
    scanName ((name n).toList ++ rest) = some (n, rest) := by
  unfold name
  by_cases hi : isIdentifier n = true
  · rw [if_pos hi]
    show scanName (n.toList ++ rest) = _
    cases hn : n.toList with
    | nil =>
      unfold isIdentifier at hi
      rw [hn] at hi
      simp at hi
    | cons c tail =>
      have hcq : c ≠ '"' := by
