...
before checking). The decode fns are `@[guest_std]` — the
bounded-Nat rationale lives in Codec.lean's `decVarNat` mark (the
varint reassembly is the only Nat arithmetic in the lane; the
depth-cap fuel is match-only). The tag `ofNat?` dispatchers are NOT
marked (the reverse!): they are the lane's INTERNAL tag switches —
the wasm-gen closure fixpoint compiles them WITHOUT exporting, and a
mark would make each a core EXPORT target (four same-named `of-nat?`
exports failed the module encode). The ENCODE half stays host-side:
the host emits the witness, the guest only reads it.
...
-- All four tag families below are generated by SchemaLang.EnumWire's
-- `declare_wire_tags` (INT-TAG mode: ctor order 0..k-1, closed at the
-- declaration — reordering is a wire-breaking change; the Tests golden
-- is the tie), each `deriving Repr, BEq, DecidableEq` with `toNat` /
-- `ofNat?` / the `ofNat?_toNat` round-trip theorem. The generated
-- `ofNat?` dispatchers are deliberately UNMARKED (no `@[guest_std]`):
-- they are the decode lane's INTERNAL tag dispatchers — the wasm-gen
-- closure fixpoint compiles them (the List-helper precedent) WITHOUT
-- exporting; a mark made each a core EXPORT target, and four
-- same-named `of-nat?` exports failed the module encode.

declare_wire_tags WU64Tag where lit | col | strlenCol

declare_wire_tags WBoolExprTag where gt | eq | and | not

declare_wire_tags WPropTag where valid | eqU | chain

declare_wire_tags WProofTag where byEval | byValidEval | steps
