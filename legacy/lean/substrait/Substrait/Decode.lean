...
- `Substrait.Decode.Inversions` — the extension/version ROW inversions,
  GENERATED by `Substrait.Meta.Inversion`'s `declare_inversion` (the
  old-name corollaries keep the axiom gate's `#print axioms` list live).
...
public import Substrait.Decode.Inversions
