...
`Substrait.ProtoGen` wire codec is generated from.
...
Generated Lean lives in `../Substrait/ProtoGen/` (GENERATED — DO NOT EDIT).
To regenerate:

```bash
# Build the protoc-gen-lean4 plugin (needs the Substrait dev shell).
lake build Plugin   # in the Lean-zh/protobuf checkout

protoc \
  --plugin=protoc-gen-lean4=<path-to-plugin> \
  --lean4_out=<Substrait>/Substrait/ProtoGen \
  --lean4_opt=lean4_prefix=Substrait.ProtoGen \
  --proto_path=<Substrait>/protos \
