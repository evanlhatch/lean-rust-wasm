...
`double/1,is-big/1,adder/2,double-area/1,run-paps/1,total/3,pick/3,str-len-demo/1,greet/1,get-user/1,watch-counts/1,watch-users/1,user-valid/4,order-error-valid/2,user-complete/4,verify-witness/1,place-order/1`
...
| `place-order` | 1 | 22 | `result-err-channel`, `fault-typed-error`, `variant-result`, `negative-empty-cart` |

## Batch coverage (one shared context, six probe batches)

| batch | probes | adds |
| --- | --- | --- |
| `grid` | 314 | the fixed u64s grid × all 17 exports + the pinned validator negatives + the W9.6 witness fixtures |
