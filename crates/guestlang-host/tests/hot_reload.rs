...
//!   deliberately SKIPPED here; the swap = at the quiesce point. The
//!   warm-start PATTERN DEMO (the event-log replay lane, formerly this
//!   file's third test) lives at
//!   `examples/warm_start_event_log.rs` — a demonstration, not a gate.
...
//!   (asserted below — the no-silent-migration negative control).
//!
//! Ownership: crates/guestlang-host/tests/hot_reload.rs (hot-reload lane).
//! Consolidated to the swap/arc-semantics core (the behavior gates);
//! additivity and src/** untouched. Skips if the wasm artifacts are
...
use common::{demo_component_path, spliced_mw_path};
...
impl Router {
    ...
    async fn route(
        ...
        func: &str,
        args: &[Val],
    ) -> guestlang_host::HostResult<Vec<Val>> {
        let mut slot = self.current.lock().await;
