...
use common::{fail, fixture, universe_snapshot_path};

use wit_parser::{Function, FunctionKind, Resolve, Type, TypeDefKind};
...
fn committed_universe() -> hostgen::Universe {
    let text = std::fs::read_to_string(universe_snapshot_path())
...
fn generated_surface_covers_the_contract() {
    ...
    let coverage = hostgen::validate_committed(
        &std::fs::read_to_string(universe_snapshot_path())
            .unwrap_or_else(|e| fail(&format!("reading the committed snapshot: {e}"))),
...
fn tampered_committed_snapshot_is_caught() {
    let text = std::fs::read_to_string(fixture("universe.snapshot.tampered"))
