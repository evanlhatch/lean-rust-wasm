...
//!     loaded via `SteelEngine::load_component` (wasmtime
...
use steel_host::schema::{self, EXPECTED_DEMO_SURFACE};
...
/// The compiled demo component (the guest the host loads).
fn demo_component_path() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../lean/wasm-backend/target/demo.component.wasm")
}
...
fn truncated_guest_component_fails_cleanly_at_every_cut() {
    let Ok(path) = std::fs::canonicalize(demo_component_path()) else {
        eprintln!("skipping: run `just wasm-compile` to build the demo component");
        return;
    };
    let bytes = std::fs::read(&path).unwrap_or_else(|e| fail(&format!("read guest: {e}")));
    let engine = SteelEngine::new().unwrap_or_else(|e| fail(&format!("engine: {e}")));
...
fn corrupt_guest_bytes_fail_with_a_named_error() {
    ...
    // wrong layer.
    let core_path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../lean/wasm-backend/target/demo.wasm");
