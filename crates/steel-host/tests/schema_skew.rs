...
use steel_host::schema::{self, EXPECTED_DEMO_SURFACE, StartError};
use steel_host::{CapabilitySet, ComponentRuntime, SteelEngine};
use wasmtime::component::Val;

/// The compiled demo component (the guest whose surface is checked).
fn demo_component_path() -> std::path::PathBuf {
...
fn the_hasher_matches_oracle_runner_and_coreutils() {
    ...
    // hasher) — pinning it here locks steel-host's `schema_hash` and
...
async fn lean_host_guest_surfaces_agree() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let coverage = std::fs::read_to_string(
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../lean/wasm-backend/COVERAGE.md"),
    )?;
    ...
    let Ok(path) = std::fs::canonicalize(demo_component_path()) else {
        eprintln!("skipping leg 3: run `just wasm-compile` to build the demo component");
        return Ok(());
    };
    let engine = SteelEngine::new()?;
    let component = engine.load_component(&path)?;
