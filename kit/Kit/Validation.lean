/-
# Kit.Validation — the error-ACCUMULATING applicative
...
Exclusion: no `Monad`, no `Alternative` (both would lie about
accumulation). This is 05 §4's curated-failure discipline as DATA:
where the lane needs ALL failures, the surface accumulates, never
single-fails.

Provenance: mined from
`legacy/lean/codegen-core/CodegenCore/Validation.lean` — content ported
fresh (the known-conversion-sites list in the legacy header is legacy-
tree-local and deliberately NOT ported; consumers land with their own
lanes).

Core-only (no mathlib/Batteries, no imports).

The five questions (notes/v3/01-core.md):
- root: Crossing — the check lane's result carrier (valid, or ALL the
  curated failures).
- carrier grade: `errs` is nonempty BY CONVENTION, enforced by the
  combinators (they only ever produce `errs` from a supplied error
  list — never `[]`); the accumulation laws are proved HERE, once.
- spine reading: the check stage of the check lane — the anti-early-exit
  contract every curated-failure surface shares.
- ladder rung: rung 1 — pure data + the proved accumulation laws
  (`foldlM_ok`, `foldlM_errs`, `traverse_ok` — the anti-early-exit
  pins).
- gate row: none yet — lands with the first validation-consuming lane's
  gate; the KitTests pins (positive + the early-exit negative control)
