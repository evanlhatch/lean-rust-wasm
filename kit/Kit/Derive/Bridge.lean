...
def elabDeclareBridge : CommandElab
  | stx@`(command| declare_bridge $name:ident := $chk:term, $rel:term where $[$rows:bridgeRow]*) => do
    ...
    let gateCmd : Syntax → CommandElabM Bool := fun cmdStx => do
      let snap ← get
      elabCommand cmdStx
    ...
    -- log — the #guard_msgs teeth would miss it).
    if refusedIff || refusedSnd || refusedCmp || refusedChk then
      logError m!"{({ code := eKB0009, message := refuseMsg, severity := .error } : Kit.Diag)}"
