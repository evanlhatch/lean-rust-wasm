...
watch-host:
	watchexec -r -w src -w generated -- cargo run
...
# Gateway world component: the SSOT generated/wit/gateway.wit (GENERATED by
...
wit-check:
	...
	WT="$HOME/.local/guestlang-tools/bin/wasm-tools"
	[ -x "$WT" ] || WT=wasm-tools
	"$WT" component wit generated/wit/gateway.wit > /dev/null
...
wasm-compile:
	...
	# generated/rust/observability_generated.rs in place — keep the
	# committed surface BEFORE it, so the compare after the regen can
	# prove no drift (artifact/ = the scratch dir, untracked).
	mkdir -p lean/wasm-backend/artifact
	cp generated/rust/observability_generated.rs lean/wasm-backend/artifact/observability_generated.rs.committed
	...
	if ! cmp -s <(tail -n +3 generated/rust/observability_generated.rs) \
	             <(tail -n +3 lean/wasm-backend/artifact/observability_generated.rs.committed); then
	  echo "FAIL: generated/rust/observability_generated.rs drifted from the committed surface — regenerate (just wasm-compile) + commit the observability surface"
	  exit 1
	fi
	WT="$HOME/.local/guestlang-tools/bin/wasm-tools"
	[ -x "$WT" ] || WT=wasm-tools
	"$WT" parse -g lean/wasm-backend/artifact/demo.wat -o lean/wasm-backend/artifact/demo.wasm
	"$WT" validate lean/wasm-backend/artifact/demo.wasm
	...
	"$WT" component embed -w demo generated/wit/demo-world.wit lean/wasm-backend/artifact/demo.wasm \
	  -o lean/wasm-backend/artifact/demo.embedded.wasm
	"$WT" component new lean/wasm-backend/artifact/demo.embedded.wasm \
	  -o lean/wasm-backend/artifact/demo.component.wasm
	"$WT" validate lean/wasm-backend/artifact/demo.component.wasm
...
splice-smoke:
	...
	cat > lean/wasm-backend/artifact/splice.wac <<'WAC'
	...
	export demo.greet;
	WAC
	"$WAC" compose lean/wasm-backend/artifact/splice.wac \
	  --dep guestlang:demo=lean/wasm-backend/artifact/demo.component.wasm \
	  -o lean/wasm-backend/artifact/spliced.component.wasm
	"$WT" validate lean/wasm-backend/artifact/spliced.component.wasm
...
splicer-mw:
	...
	"$WT" validate "$mw"
	[ -f lean/wasm-backend/artifact/demo.component.wasm ] || { echo "FAIL: no demo.component.wasm — run 'just wasm-compile'"; exit 1; }
...
rt-conformance:
	#!/usr/bin/env bash
	set -euo pipefail
	[ -f lean/wasm-backend/artifact/demo.wasm ] || { echo "FAIL: no demo.wasm — run 'just wasm-compile'"; exit 1; }
...
edgepython:
	...
	WT="$HOME/.local/guestlang-tools/bin/wasm-tools"
	[ -x "$WT" ] || WT=wasm-tools
	"$WT" parse lean/edgepython/artifact/py.wat -o lean/edgepython/artifact/py.wasm
	"$WT" validate lean/edgepython/artifact/py.wasm
	CC="$HOME/lean-rust-wasm/.devenv/profiles/wasm/profile/bin/cc" \
...
mutation-proof:
	...
	echo "── (a) byte-tie: append a byte to generated/rust/schema_generated.rs ──"
	f=generated/rust/schema_generated.rs
	save "$f"
	printf 'X' >> "$f"
	...
	jj restore generated/rust/observability_generated.rs 2>/dev/null || true
...
#   SIZE: lean/wasm-backend/artifact/demo.component.wasm = 52524 bytes -> budget 80000 (1.5x rounded up)
...
budget-check:
	...
	COMP=lean/wasm-backend/artifact/demo.component.wasm
...
# artifact/diff.json is deterministic (proved by 2× regen 2026-09-17).
wasm-diff-check:
	#!/usr/bin/env bash
	set -euo pipefail
	D=lean/wasm-backend/artifact/wasm-diff-check
	mkdir -p "$D"
	cp generated/wit/demo-world.wit "$D/demo-world.wit.committed"
	cp generated/rust/observability_generated.rs "$D/observability_generated.rs.committed"
	cp lean/wasm-backend/artifact/diff.json "$D/diff.json.committed"
	just wasm-compile
	cmp -s lean/wasm-backend/artifact/diff.json "$D/diff.json.committed" \
	  || { echo "FAIL: artifact/diff.json drifted — regenerate (just wasm-compile) + commit"; exit 1; }
	for f in "generated/wit/demo-world.wit" "generated/rust/observability_generated.rs"; do
	  b=$(basename "$f")
