/-
# TextKit.Diag — the ONE diagnostic envelope (notes/v3/05-codegen.md §4)

Every channel speaks it: elaboration errors, parse failures (TextKit's
`ParseError` = a Diag with position — THE CONVERGED SHAPE,
`TextKit.Error`'s `ParseError extends Diag`), lint findings, gate
verdicts, obligation rows, Rust spans, the guest's thin diagnostics.
One envelope, seven fields, a closed severity universe of five; the
did-you-mean field is filled by the ONE engine (`TextKit.suggestFor` —
TextKit.Suggest) through the `closedWorld` constructor, so a
closed-world failure cannot skip the suggestion (the discipline lands
in the SHAPE, not in a convention).
...
the envelope is the honest hand version). This module is the envelope's
HOME: the C0 module-system substrate at the lowest point every
diagnostic side imports (the cone/build call — Kit's Diag re-exports
these declarations under the `Kit` namespace, interface-preserved; the
textkit→kit direction is build-impossible: a module file cannot import
a pre-`module` file, and Kit's own module files already import TextKit,
so down is the only legal direction). The legacy TextKit.Diag's
farthest-failure lane stays in TextKit's `ParseError.farther`, where it
always lived. The severity five and the field set are the doctrine's,
verbatim (05 §4).
...
structure Diag where
  ...
  valid : List String := []
  /-- The did-you-mean, filled by the ONE engine (`TextKit.suggestFor`)
...
def closedWorld (code : ECode) (message : String) (severity : Severity)
  ...
  { code := code
    ...
    got := some got
    valid := valid
    suggest := TextKit.suggestFor got valid
    severity := severity }
