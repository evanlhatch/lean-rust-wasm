...
import SchemaLang.OracleMirror -- the mirror's DATA the emitters render (pinned against the defs below)
import Plausible
import TestKit
...
/-! ── THE MIRROR PINS (the comparison BOOKKEEPING's authority) ─────────
   crates/oracle-runner's mirror (CompareMode/ErrorId/Outcome/compare/
   classify/firstDiffAt/verdict/jsonVerdict/ser_val) is GENERATED by the
   oracle-mirror emitter — `SchemaLang.OracleMirror` (its DATA tables +
   the Rust renderer, registered in schema-lang's coreEmitters; the
   forge byte-tie pins generated/rust/oracle_mirror_generated.rs; the
   oracle runner `#[path]`-includes it). That module lives schema-lang-
   side (a `module` file cannot import this non-module root — and the
   emitters must register with the forge byte-tie's registry), so the
   AUTHORITY lands here: the sweeps below prove the DATA agrees with
   THIS module's definitions —
     * `compareTableRows` vs `CompareMode.compare` over the whole
       finite comparison space (all modes x all error options x payload
       samples);
     * `classifyRows` vs `classify` over every error pair;
     * the display tables vs the ToString instances + the mode names.
   A drift in the mirror data fails THIS build, not a downstream gate.
   The INDEPENDENCE lives in the EVALUATION: the replay machinery (the
   arg builders, the stream drain, the component call — oracle-runner's
   driver) is NOT generated, and ser_val's Val reads stay host-side —
   only the ser-FORM CONTRACT strings ride as data.
-/
namespace OracleMirror -- the pins' helpers (Oracle.lean's own ctor sets)

/-- The CompareMode ctors, in declaration order. -/
def compareModes : List CompareMode := [.ignore, .identity, .full]

/-- The ErrorId ctors, in declaration order. -/
def errorIds : List ErrorId := [.unknownFn, .arityDrift, .trap]

/-- The DivergenceClass ctors, in declaration order. -/
def divergenceClasses : List DivergenceClass :=
  [.valueMismatch, .expectedValueGotError, .expectedErrorGotValue,
   .errorIdentityMismatch, .errorPayloadMismatch]

/-- The mode's ctor NAME (the mirror data's mode key). -/
def compareModeName : CompareMode → String
  | .ignore => "ignore"
  | .identity => "identity"
  | .full => "full"

/-! THE DISPLAY PINS: the mirror's name tables ARE the ToString
    instances (+ the mode names) — a display change is a build error
    HERE, never a silent mirror drift. -/
#guard (errorIds.zip SchemaLang.OracleMirror.errorIdNames).all
  (fun (e, n) => n == toString e)
#guard (divergenceClasses.zip SchemaLang.OracleMirror.divergenceClassNames).all
  (fun (c, n) => n == toString c)
#guard (compareModes.zip SchemaLang.OracleMirror.compareModeNames).all
  (fun (m, n) => n == compareModeName m)

/-! THE COMPARE PIN: the mirror's truth table agrees with the
    definition's `CompareMode.compare` over the whole finite comparison
    space (the error identities as their NAMES — exactly how the Rust
    side compares them). A re-ordered, weakened, or added arm fails
    the build. -/
#guard
  let err : List (Option ErrorId) := none :: errorIds.map some
  let errName (o : Option ErrorId) : Option String := o.map toString
  let payloads : List String := ["", "a", "b", "42"]
  compareModes.all fun m =>
    err.all fun e => err.all fun g =>
      payloads.all fun pe => payloads.all fun pg =>
        SchemaLang.OracleMirror.mirrorCompareByName (compareModeName m) (errName e) (errName g) pe pg ==
          CompareMode.compare m { error := e, payload := pe } { error := g, payload := pg }

/-! THE CLASSIFY PIN: the mirror's classification table agrees with
    `classify` over every error pair (its classes as their NAMES). -/
#guard
  let err : List (Option ErrorId) := none :: errorIds.map some
  err.all fun e => err.all fun g =>
    SchemaLang.OracleMirror.mirrorClassifyByName (e.map toString) (g.map toString) ==
      (toString (classify { error := e, payload := "" } { error := g, payload := "" }))
