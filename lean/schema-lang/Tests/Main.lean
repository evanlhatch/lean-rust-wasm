...
def deltaChecks : CheckResult := do
  ...
  -- 6.5.3: the emitter audits its OWN output — banned/required patterns
  -- over the emitted text (GateKit recipe). The same rules must hold for
  -- every generated WIT artifact; drift into a banned construct fails CI.
  _ ← assert "deltaWit self-audit"
      (GateKit.auditFindings
        [ { name := "no-todo", pattern := "TODO"
          , why := "unfinished emission leaked into a generated artifact" }
        , { name := "no-fixture", pattern := "fixme", why := "same" }
        , { name := "generated-mark", pattern := "// GENERATED by schema-lang"
          , required := true
          , why := "the DO-NOT-EDIT banner is what gen-check enforces" } ]
        wout |>.isEmpty)
  .ok ()
...
@[schema_fn]
def probeDefaultFn (x y : UInt32) : UInt32 := x
