...
def TableObligation.holdsOn (o : TableObligation)
...
/-- THE DISCHARGE. A `decidableNow` row's evidence is the kernel's
    verdict over the PROVIDED table; the other four rungs are the loud
    gap — a hand-set tier the lane cannot name (the module header's
    rung-by-rung reading), a FALSE verdict (a sabotaged table), or no
    provided table — discharge refuses, it does not fabricate
    evidence. -/
...
theorem TableObligation.discharge_decidableNow_of_holds (o : TableObligation)
  ...
  have h' : o.payload.agg.check t = true := h
  simp [TableObligation.discharge, ht, h']
