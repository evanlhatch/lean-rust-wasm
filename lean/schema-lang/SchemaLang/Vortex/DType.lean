...
Deliberately omitted (containment, lean-v3 Part 10): `FieldMask`/`Field`
projections, visitor/downcast traits, sessions/plugins, `Arc`-sharing.
The fork's `PartialEq` is structural (ptr_eq is a shortcut, not a
semantics), so the derived `BEq` here is faithful — including
`Extension`: `ExtDTypeRef`'s equality is id + metadata-bytes + storage,
exactly the erased triple modeled here.

The drift tie (oracle doctrine): this model and the Rust enum agree by
the serde handshake — Lean → bytes → Rust parse, and back — proved on
...
/-- Fork `Nullability`: two values (unlike Substrait's three).
    NonNullable is the fork's `#[default]`. -/
inductive Nullability where
  | nonNullable | nullable
deriving Repr, BEq, DecidableEq, Inhabited

/-- Fork `PType` — the in-memory physical layout enum. Discriminants are
    wire-stable (proto/flatbuffer tags). -/
...
/-- The name map is injective and total over the 11 variants — the
    emitted match has no collisions or gaps. -/
theorem PType.engineName_inj : Function.Injective engineName := by
  intro a b h
  cases a <;> cases b <;> simp [engineName] at h ⊢ <;> exact h

/-- Byte width of the physical layout (f16 = 2). -/
def PType.byteWidth : PType → Nat
  | .u8 | .i8 => 1
  | .u16 | .i16 | .f16 => 2
  | .u32 | .i32 | .f32 => 4
  | .u64 | .i64 | .f64 => 8
...
/-- The i256 bounds (fork: MAX_PRECISION = MAX_SCALE = 76). -/
...
mutual
/-- Well-formedness: the checks the fork enforces at construction that
    are expressible without the registry (decimal bounds). -/
