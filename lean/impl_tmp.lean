...
resolution chain cannot drift from the row:
    ...
    declare_inversion inv_urnLine where
      ...
      line := [tok Grammar.indentUnit "  " lit "@" anchor a
               tok Grammar.colonSpTok ": " name urn]
      steps := [reduce parseUrnEntryBody.eq_1 scan a expect colonTok finish]
...
The `cor` corollary keeps the pre-macro theorem name live — the axiom
...
syntax (name := invLit) "lit " str : inversionSeg
syntax (name := invTok) "tok " ident str : inversionSeg
syntax (name := invPfx) "pfx" : inversionSeg
syntax (name := invDot) "dot" : inversionSeg
syntax (name := invAnchor) "anchor " ident : inversionSeg
syntax (name := invNum) "num " ident : inversionSeg
syntax (name := invName) "name " ident : inversionSeg

/-- A resolution step (see the module header). -/
declare_syntax_cat inversionStep

syntax (name := invReduce) "reduce " term : inversionStep
syntax (name := invScan) "scan " ident : inversionStep
syntax (name := invPeel) "peel " str : inversionStep
syntax (name := invExpect) "expect " ident : inversionStep
syntax (name := invFinish) "finish" : inversionStep
syntax (name := invTail) "tail" : inversionStep

/-- The optional `cor <oldName>` corollary block. -/
declare_syntax_cat inversionCor
syntax (name := invCor) "cor " " := " ident : inversionCor

/-- The optional `proof := by …` bespoke tail. -/
declare_syntax_cat inversionProof
syntax (name := invProof) "proof " " := " term : inversionProof
...
declare_inversion <newName> where
  parse := <parser term>
  emit := <emitter application>
  binders := <bracket binder list>
  result := <the `some` payload>
  line := [<segment>*]
  steps := [<step>*]
  cor := <old-name corollary>      (optional)
  proof := by <tail>               (optional; only read with a `tail` step)
```
-/
...
/-- The materialized row: `'c' :: ` conses concatenate, `.toList` pieces
    join with ` ++ `; the result is defeq to any hand-written parenthesized
    form. -/
...
/-- The header reveal rewrites for the LEADING segments (up to the first
    anchored/numbered field): one `rw [show <atom>.toList = [...] from by
    decide]` per literal run. -/
...
def generated (thmName : String) (parseFn emitFn res : String) (emitFnName : String)
    (bt args : String)
  ...
  let body ← if version then versionBody segs stepTxts tailProof
    else anchorBody parseFn emitFn emitFnName segs stepTxts
...
def elabDeclareInversion : CommandElab := fun stx => do
  ...
  | `(declare_inversion $thm:ident where
      ...
      binders := $bs*
      result := $resT:term
      line := [$segs:inversionSeg*]
      steps := [$stps:inversionStep*]
      $[$corB?:inversionCor]? $[$proofB?:inversionProof]?) => do
    -- the file map for source slicing
    let fm ← getFileMap
    let srcOf (s : Syntax) : CommandElabM String := do
      match s.getPos?, s.getTailPos? with
      | some a, some b => pure (fm.source.extract a b)
      | _, _ => pure (toString s)
    let parseS ← srcOf parseT
    let emitS ← srcOf emitT
    let resS ← srcOf resT
    let emitFnS ← srcOf (rawAppFn emitT)
    -- binder texts + argument list
    let mut binderTxts : Array String := #[]
    let mut argTxts : Array String := #[]
    for b in bs do
      ...
      for i in ids do
        let s ← srcOf i
        argTxts := argTxts.push s
        binderTxts := binderTxts.push s
      match ty with
      | some t =>
        let ts ← srcOf t
        let n := (binderTxts.size - ids.size).toNat
        runner0 := ()
      | none => pure ()
    pure ()
