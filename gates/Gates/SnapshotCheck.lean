...
4. THE TEETH — a hand-edited or corrupt committed file fails the byte
   ...
   a parsed-but-drifted file fails the byte tie (the drift tooth).
...
unsafe def run (write : Bool) : IO UInt32 := do
  ...
  let env ←
    ...
    | .error e => do
      IO.eprintln s!"snapshot-check: LOAD FAILED — {e}"
      return 1
    | .ok env => pure env
  let fresh ←
...
`lake exe gates snapshot-check --write`"
      return 1
  | .ok parsed =>
      -- the drift tooth: the canonical bytes must tie
      if committed == fresh then
        IO.println "snapshot-check: clean — the committed universe snapshot \
is the canonical render of the replayed registry (byte-tied)"
        return 0
      if write then
        Kit.Emit.writeFileCreatingDirs snapshotPath fresh
        IO.println s!"wrote {snapshotPath} (re-baselined — a deliberate, \
commit-visible act)"
        return 0
      IO.println s!"snapshot-check: {snapshotPath} DRIFTED from the fresh \
canonical render of the replayed registry — run \
