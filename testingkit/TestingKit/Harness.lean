/-
# TestingKit.Harness — the check runner + the verdict rendering + the driver
...
Mined from: legacy/lean/TestingKit/TestingKit/PropSpec.lean (runIO's three-way
...
- root: none — the verdict machinery over TestingKit.Spec's suites.
...
/-- The shrink evidence appended to a failing instance's message (08
    §11: failing sweeps report the minimal counterexample + the shrink
    path). The attachment's `fails` must reproduce the prop's failure at
    the drawn value; a misdeclared attachment DECLINES loudly — the
    message says so, it never silently pretends to have shrunk. An
    unattached spec's message is byte-identical to before. -/
def shrunkNote (sp : Spec) (i : Nat) (e : String) : String :=
  match sp.shrunk with
  | none => e
  | some sh =>
      let v0 := sh.draw (instTape sp.seed i)
      if sh.fails v0 then
        let trail := shrinkTrail sh.wv sh.fails v0
        e ++ " | shrunk: minimal " ++ sh.render (shrinkLoop sh.wv sh.fails v0) ++
             ", path [" ++ String.intercalate " → " (trail.map sh.render) ++ "]"
      else e ++ " | shrink attachment declined: the drawn value does not reproduce the failure"
...
/-- Run one spec — pure, structured: the seeded sweep (the prop must hold
    ...
    failing property — but a failing property is reported first). A
    failing instance's evidence carries the shrink attachment's minimal
    case + path when present. -/
def Spec.run (sp : Spec) : Verdict :=
  match firstFailure sp.seed sp.numInst sp.prop with
  | some (i, e) => .fail .prop i (replaySeedOf sp.seed i) (shrunkNote sp i e)
  | none =>
